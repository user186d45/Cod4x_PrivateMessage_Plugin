...
PCL void OnClientEnterWorld(client_t* client) {
	if(isPlayerOnline(Plugin_GetPlayerName(NUMFORCLIENT(client))) == 1) {
		Plugin_Printf("Private Message Plugin: Player already exists.\n");
		return;
...
__cdecl int initDb() {
	...
	Plugin_Printf("Private Message Plugin: Database Initialized successfully.\n");
...
__cdecl int writePlayerOnConnect(client_t* client) {
	...
	Plugin_Printf("Private Message Plugin: successfully wrote the player info to database.\n");
	sqlite3_finalize(stmt);
	pthread_mutex_unlock(&mutex);
	return 0;
...
__cdecl int deletePlayerOnDC(client_t* client) {
	...
	Plugin_Printf("Private Message Plugin: Player successfully removed from the database.\n");
	sqlite3_finalize(stmt);
	pthread_mutex_unlock(&mutex);
...
__cdecl void privateMessage() {
	...
	char msg[512];
	for(int i = 2, j = 0; i < Plugin_Cmd_Argc(); i++) {
